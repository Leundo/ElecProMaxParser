...
class Rlt(str, Enum):
    ...
    def first_targets(self) -> List[str]:
        if self.isX():
            ...
            return ['发电机ID号', '发电机名称', '拓扑着色', '有功值', '无功值', '电压值', '厂站ID号', '发电机类型', 'BPA名', '机组识别码']
        elif self == Rlt.fuhe:
            return ['负荷ID号', '负荷名称', '拓扑着色', '有功值', '无功值', '厂站ID号', 'BPA名']
        elif self == Rlt.muxian:
...
def get_bundle_paths(subname: str) -> List[str]:
    bundle_paths = [ file.path for file in os.scandir(os.path.join(constant.data_folder_path, subname)) if file.is_dir() ]
    return bundle_paths
